...
class App {
    ...
    setupButtonListeners() {
        const { newGameBtn, loadGameBtn, settingsBtn } = this.elements;
        ...
        // Store the reference to the handler so we can remove it later
        ...
        try {
            if (this.isRunning) {
                if (typeof window.Module._emscripten_pause_main_loop === 'function') {
                    ...
                    logger.log('Simulation paused', 'warning');
                ...
                if (typeof window.Module._emscripten_resume_main_loop === 'function') {
                    ...
                    logger.log('Simulation resumed', 'success');
                    const { pauseBtn } = this.elements;
    ...
    async resetSimulation() {
        ...
        try {
            ...
            this.grid = {
                cellSize: 20, // Size of each grid cell in pixels
                width: Math.ceil(canvas ? canvas.width / 20 : 40),
                height: Math.ceil(canvas ? canvas.height / 20 : 30),
                cells: []
            };
            ...
            // Re-add keyboard event listener if we're still in the game
            if (this.elements.gameContainer && !this.elements.gameContainer.classList.contains('hidden')) {
